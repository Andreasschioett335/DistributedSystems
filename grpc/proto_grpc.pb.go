...
type ChitChatClient interface {
	...
	Chat(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ClientMessage, ServerEvent], error)
}
...
func (c *chitChatClient) Chat(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ClientMessage, ServerEvent], error) {
...
type ChitChat_ChatClient = grpc.BidiStreamingClient[ClientMessage, ServerEvent]
...
type ChitChatServer interface {
	// Client opens a bidirectional stream for sending and receiving messages.
	Chat(grpc.BidiStreamingServer[ClientMessage, ServerEvent]) error
...
func (UnimplementedChitChatServer) Chat(grpc.BidiStreamingServer[ClientMessage, ServerEvent]) error {
...
func _ChitChat_Chat_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ChitChatServer).Chat(&grpc.GenericServerStream[ClientMessage, ServerEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ChitChat_ChatServer = grpc.BidiStreamingServer[ClientMessage, ServerEvent]
